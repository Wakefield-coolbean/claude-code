// Benchmark world generation: node tools/worldgen-bench.mjs [seed] [size] [type|nether] [centerChunkX] [centerChunkZ]
...
const gen = new WorldGenerator(seed, type === 'nether' ? { dimension: 'nether' } : { type });
...
const fort = gen.findNearestFortress ? gen.findNearestFortress(0, 0) : null;
...
if (fort) console.log(`nearest fortress to 0,0: ${JSON.stringify(fort)}`);
